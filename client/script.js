// Full JS: levels reveal on Enter, course tabs above questions, dark mode, rendering logic

// --- SAMPLE COURSES OBJECT (100L and 200L keys required) ---
const courses = {
  // 100L
 phy102: [
    {
...
// --- State: which levels revealed (0=none,1=100L,2=100L+200L) ---
let levelState = 0;

// UI elements
const instr = document.getElementById('level-instruction');
const level100 = document.getElementById('level-100');
const level200 = document.getElementById('level-200');
const questionsSection = document.getElementById('questions');
const reveal100Btn = document.getElementById('reveal-100');
...
// reveal next level when Enter pressed
function revealNextLevel() {
  if (levelState === 0) {
    showLevel(level100);
    levelState = 1;
    instr.textContent = '100L revealed — Press Enter to reveal 200L courses';
    // add a manual reveal button for 200L
    const btn = document.createElement('button');
    btn.className = 'inline-btn';
    btn.id = 'reveal-200';
    btn.textContent = 'Reveal 200L';
    btn.onclick = () => revealNextLevel();
    instr.appendChild(btn);
    // focus first tab
    setTimeout(() => {
      const first = level100.querySelector('.tab-btn');
  ...
  if (levelState === 1) {
    showLevel(level200);
    levelState = 2;
    instr.textContent = 'Both levels revealed';
    // remove instruction after short delay
    setTimeout(()=> instr.style.display='none', 1400);
    return;
  }
}

// keyboard listener: Enter to reveal levels
...
// also allow clicking the reveal-100 button
if (reveal100Btn) reveal100Btn.addEventListener('click', revealNextLevel);

// Tab switching logic (event delegation)
document.body.addEventListener('click', (e) => {
  const btn = e.target.closest('.tab-btn');
  if (!btn) return;
  // ensure level is visible before allowing selection
  const parentLevel = btn.closest('.level');
  if (parentLevel && parentLevel.getAttribute('aria-hidden') === 'true') return;
  ...
  // render questions for chosen course
  const courseKey = btn.dataset.course;
  if (courseKey) {
    renderQuestions(courseKey);
    showQuestionsSection();
    // scroll into view smoothly
    setTimeout(()=> questionsSection.scrollIntoView({ behavior:'smooth', block:'start' }), 80);
  }
});
...
function renderQuestions(courseKey, searchTerm = "") {
  ...
  search.style = 'width:100%;max-width:360px;padding:8px;margin:8px 0;border-radius:6px;border:1px solid #ccc;';
  questionsSection.appendChild(search);
  search.addEventListener('input', () => renderQuestions(courseKey, search.value));
  ...
  questions.forEach((q) => {
    ...
    card.innerHTML = `<strong>Q${q.number}:</strong> ${q.question}`;
    ...
    (q.options || []).forEach(opt => {
      ...
      button.addEventListener('click', () => {
        // reset styles
        ul.querySelectorAll('.option-btn').forEach(b=> b.classList.remove('correct','incorrect','selected'));
    ...
    ans.innerHTML = `<strong>Answer:</strong> <span style="color:green">${q.answer||''}</span>`;
    showBtn.addEventListener('click', () => {
      ans.style.display = ans.style.display === 'block' ? 'none' : 'block';
      showBtn.textContent = ans.style.display === 'block' ? 'Hide Correct Answer' : 'Show Correct Answer';
    });
...
if (darkToggle) darkToggle.addEventListener('click', ()=> setDarkMode(!document.body.classList.contains('dark-mode')));